...
class Node2Vec:
    ...
    def train(self, workers=4, is_loadmodel=False, is_loaddata=False):
        ...
        w2v.save(path + '/Node2Vec_a=0.1_b=0.1.model')
        print('Training Done.')

        return w2v
    
def constructHNet(train_mirna_disease_matrix, mirna_matrix, disease_matrix):
    # mirna_matrix=np.where(mirna_matrix> 0.5, 1, 0)
    # disease_matrix=np.where(disease_matrix> 0.5, 1, 0)
    ...
    return np.where(mat3> 0.5,1, 0)



def loaddata():
  adj_matrix=np.loadtxt("./data/DiseaseAndRNABinary.csv", dtype=int, delimiter=",")
  adj_matrix=adj_matrix.T
  RNASimi1 = loadmat('./data/RNASi_kernel_0.1.mat')
  lncSimi = RNASimi1['RNASi_kernel']
  disSimi1 = loadmat('./data/DiSi_kernel_0.1.mat')
  disSimi = disSimi1['DiSi_kernel']
  adj_matrix=constructHNet(adj_matrix,lncSimi,disSimi)
  positive_index_tuple = np.where(adj_matrix == 1)
...
if __name__ == '__main__':
    main()
    # print(node2vec.wv["250"])
    # print(len(node2vec.wv.key_to_index.keys()))
    #node2vec = Word2Vec.load("../data/Node2Vec.model")
