...
if __name__ == '__main__':
    ...
    A = np.loadtxt('./datasets/data1/DiseaseAndRNABinary.csv',delimiter=",")
    A = A.T
    results = {} 
    disSimi1 = loadmat('./datasets/data1/DiSi_kernel_0.9.mat') 
    disSimi = disSimi1['DiSi_kernel']
    RNASimi1 = loadmat('./datasets/data1/RNASi_kernel_0.8.mat')
    ...
    for i in range(0, len(positive_index_list), positive_split):
        ...
        for epoch in range(args.epochs):
            train_predict_result, train_lable = model(x,edge_index, new_lncrna_disease_matrix_tensor,
                                                       train_model=True)
            # print(train_predict_result.shape)
            # print(train_lable.shape)
            loss = F.binary_cross_entropy(train_predict_result.to(torch.float32), train_lable)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        for num in range(test_predict_result.size()[0]):
            ...
            prediction_matrix[row_num, col_num] = test_predict_result[num]
    ...
    mean_cross_tpr = normalize(tpr_arr)  # axis=0
    mean_cross_fpr = normalize(fpr_arr)
    mean_cross_recall = normalize(recall_arr)
    mean_cross_precision = normalize(precision_arr)
    mean_cross_accuracy = normalize(accuracy_arr)
    ...
    AUPR = np.trapz(np.mean(precision_arr, axis=0), np.mean(recall_arr, axis=0))
